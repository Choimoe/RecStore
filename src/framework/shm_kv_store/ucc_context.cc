...
UCCContext::UCCContext() {
  ...
  if (!rank_str || !size_str) {
    throw std::runtime_error("UCC requires MPI environment variables (e.g., "
                             "OMPI_COMM_WORLD_RANK/SIZE)");
  }
  ...
  check_ucc_status(ucc_team_create_post(&context_, 1, &team_params, &team_),
...
void UCCContext::Barrier() {
  ...
  check_ucc_status(ucc_collective_init(&args, &request, team_),
                   "ucc_collective_init for barrier");
  check_ucc_status(
      ucc_collective_post(request), "ucc_collective_post for barrier");
...
std::vector<char> UCCContext::Allgatherv(const std::vector<char>& send_buffer) {
  ...
  check_ucc_status(ucc_collective_init(&args, &req, team_),
                   "ucc_collective_init for allgather (counts)");
  check_ucc_status(
      ucc_collective_post(req), "ucc_collective_post for allgather (counts)");
  ucc_status_t status;
  ...
  check_ucc_status(ucc_collective_init(&args, &req, team_),
                   "ucc_collective_init for allgatherv");
  check_ucc_status(
      ucc_collective_post(req), "ucc_collective_post for allgatherv");
  while (UCC_INPROGRESS == (status = ucc_collective_test(req))) {
  }
  ucc_collective_finalize(req);
