...
class RenderYcsbPagesTest(unittest.TestCase):
    def test_merges_history_and_writes_dashboard(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("Run ID", js)
            self.assertIn("summary.csv", js)
            self.assertIn("recstore-ycsb-theme", js)
            self.assertIn("Latest Change", js)
            self.assertIn("run_throughput_ops_sec", js)
            self.assertIn('fetch("latest/run.json"', js)
            self.assertIn('fetch("history.jsonl"', js)
